...
// 폼 제출 처리
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_script'])) {
    // 기본 정보 검증
    ...
    $script_id = $conversion_model->create_conversion_script($script_data);
    
    if ($script_id) {
        alert('새 전환 스크립트가 성공적으로 생성되었습니다.', 'conversion_script_view.php?id=' . $script_id . '&msg=스크립트가 성공적으로 생성되었습니다.&msg_type=success');
        exit;
    } else {
        alert('스크립트 생성 중 오류가 발생했습니다. 다시 시도해주세요.', '');
        exit;
    }
}

// 기본 스크립트 코드 템플릿
$default_script_code = <<<EOT
// 기본 추적 코드
(function(w,d,s,l,i){w[l]=w[l]||[];
    var f=d.getElementsByTagName(s)[0],
        j=d.createElement(s),dl=l!='cfLayer'?'&l='+l:'';
    j.async=true;j.src='{$cf_url}/js/cf.js';
    f.parentNode.insertBefore(j,f);
})(window,document,'script','cfLayer','{$campaign['campaign_hash']}');
...
// 기본 설치 안내
$default_installation_guide = '1. 웹사이트의 모든 페이지에 기본 추적 코드를 삽입하세요. 코드는 <head> 태그 또는 <body> 태그의 끝부분에 넣을 수 있습니다.

2. 전환 발생 시(예: 구매 완료 페이지), trackConversion() 함수를 호출하여 전환을 추적하세요.

예시: 구매 완료 페이지에서 주문 금액을 전달
```javascript
trackConversion(50000); // 50,000원 구매 완료
```';
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">새 전환 스크립트 생성</h1>
        <div>
            <a href="<?php echo CF_ADMIN_URL; ?>/campaign/campaign_view.php?id=<?php echo $campaign_id; ?>" class="d-none d-sm-inline-block btn btn-sm btn-secondary shadow-sm">
        ...
        <div class="col-lg-12">
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">새 전환 스크립트 정보</h6>
                </div>
                <div class="card-body">
                    <form method="post" action="" id="scriptForm">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control code-editor" id="script_code" name="script_code" rows="12" required><?php echo htmlspecialchars($default_script_code); ?></textarea>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control" id="installation_guide" name="installation_guide" rows="5"><?php echo htmlspecialchars($default_installation_guide); ?></textarea>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-10 offset-sm-2">
                                <button type="submit" name="create_script" class="btn btn-primary">
                                    <i class="fas fa-plus"></i> 스크립트 생성
                                </button>
                                <a href="<?php echo CF_ADMIN_URL; ?>/campaign/campaign_view.php?id=<?php echo $campaign_id; ?>" class="btn btn-secondary">
        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <h5 class="mt-4">전환 유형 예시</h5>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="card mb-3">
                                <div class="card-header">구매 완료 전환</div>
                                <div class="card-body">
                                    <p>구매 완료 페이지에서 주문 금액을 추적합니다.</p>
                                    <pre><code class="language-javascript">// 구매 완료 페이지에 적용
trackConversion(50000); // 50,000원 구매 완료</code></pre>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card mb-3">
                                <div class="card-header">회원 가입 전환</div>
                                <div class="card-body">
                                    <p>회원 가입 완료 페이지에서 추적합니다.</p>
                                    <pre><code class="language-javascript">// 회원 가입 완료 페이지에 적용
cfLayer.push({
    event: 'conversion',
    conversionType: '회원가입',
    conversionValue: 0
...
<script src="<?php echo CF_ADMIN_URL; ?>/assets/vendor/codemirror/lib/codemirror.js"></script>
<script src="<?php echo CF_ADMIN_URL; ?>/assets/vendor/codemirror/mode/javascript/javascript.js"></script>
<script src="<?php echo CF_ADMIN_URL; ?>/assets/vendor/codemirror/addon/edit/matchbrackets.js"></script>
<script>
$(document).ready(function() {
    ...
    // 전환 유형 변경 시 스크립트 코드 업데이트
    $('#conversion_type').change(function() {
        var type = $(this).val();
        var currentCode = editor.getValue();
        
        // conversionType 값 업데이트
        var updatedCode = currentCode.replace(/conversionType: '[^']*'/, "conversionType: '" + type + "'");
